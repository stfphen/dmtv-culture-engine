...
const AI_IMAGE_PROMPT = q => `Cinematic, moody, on-brand culture editorial image: ${q}. Dark, high-contrast, flash-photography nightlife aesthetic, oversized silhouettes, no text, no logos, no watermarks.`;

// Google Gemini 2.5 Flash Image ("Nano Banana"). Returns an inline data URI.
async function fromGemini(query) {
  const key = process.env.GEMINI_API_KEY; if (!key) return null;
  const model = process.env.GEMINI_IMAGE_MODEL || "gemini-2.5-flash-image";
  try {
    const r = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${key}`, {
      method: "POST", headers: { "content-type": "application/json" },
      body: JSON.stringify({ contents: [{ parts: [{ text: AI_IMAGE_PROMPT(query) }] }] }),
    });
    if (!r.ok) { console.warn("[gemini] image error", r.status); return null; }
    const d = await r.json();
    const part = (d.candidates?.[0]?.content?.parts || []).find(p => p.inline_data || p.inlineData);
    const inline = part?.inline_data || part?.inlineData; if (!inline?.data) return null;
    const mime = inline.mime_type || inline.mimeType || "image/png";
    return { provider: "gemini", image_url: `data:${mime};base64,${inline.data}`, attribution: "AI-generated (DMTV \u00b7 Gemini)", rights: "original_ai", publishable: true, query };
  } catch (e) { console.warn("[gemini] failed:", e.message); return null; }
}

async function fromOpenAIImage(query) {
  const key = process.env.OPENAI_API_KEY; if (!key) return null;
  try {
    const r = await fetch("https://api.openai.com/v1/images/generations", {
      method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${key}` },
      body: JSON.stringify({ model: process.env.IMAGE_MODEL || "dall-e-3", prompt: AI_IMAGE_PROMPT(query), size: "1024x1024", n: 1 }),
    });
    if (!r.ok) return null;
...
// AI-original image: prefer Gemini ("Nano Banana"), else OpenAI. Honors IMAGE_PROVIDER override.
async function fromAI(query) {
  const pref = (process.env.IMAGE_PROVIDER || "").toLowerCase();
  if (pref === "openai") return (await fromOpenAIImage(query)) || (await fromGemini(query));
  if (pref === "gemini") return (await fromGemini(query)) || (await fromOpenAIImage(query));
  return (await fromGemini(query)) || (await fromOpenAIImage(query));
}
